...
function sendOTPEmail($email, $otp, &$errorMsg = '') {
    ...
    try {
        ...
        $mail->setFrom('appasivaishnavi@gmail.com', 'online Campus selection System');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['send_otp'])) {
        ...
        $stmt = $con->prepare("SELECT id FROM company WHERE email = ?");
        ...
        if (
            ...
            $stmt = $con->prepare("UPDATE company SET password = ? WHERE email = ?");
            ...
            if ($stmt->execute()) {
                ...
                $show_otp_form = false;
                header("refresh:2; url=company_login.php");
            } else {
